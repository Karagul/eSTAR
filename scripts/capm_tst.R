...
prc2rtnTS <- function(rsx) {
  tsDate=as.Date(as.character(rsx$pbDate),"%Y%m%d")
  rsd  <- xts(rsx,tsDate,frequency=365)
  tsx <- sapply(rsd,function(x) diff(log(x)))
  #OR tsx <- sapply(rsd,function(x) (diff(x)/lag(x)) )
  return(tsx)
}
...
prc2rtn <- function(rsx) {
  tsDate=as.Date(as.character(rsx$pbDate),"%Y%m%d")
	...
	#OR xd <- sapply(rsx[,2:ncol(rsx)],function(x) (diff(x)/x))
